  ...
  <div class="col">
    ...
    <div class="field" style="margin-top:4px;">
      ...
      <div class="mode-switch">
        ...
        <button type="button" class="mode-btn" id="mode-btn-royale" onclick="setGameMode('royale')" style="grid-column:1/-1;">🪂 БАТЛ РОЯЛЬ</button>
      </div>
    </div>
...
<div id="royale-overlay">
  ...
  <div id="royale-main">
    ...
    <div id="royale-stage">
      <div id="royale-map-bg"></div>
      <div id="royale-grid"></div>
    </div>
    <div class="royale-side">
...
function rsoDrawPrep() {
  ...
  ctx.font = '14px Inter, sans-serif'; ctx.fillStyle = '#ccc';
  ctx.fillText('Бойцов: ' + rsoFighters.length + ' · Оружие: AK-47', rsoW/2, rsoH/2 - 34);
  ctx.font = '12px monospace';
  rsoWrap(ctx, rsoFighters.map(f => f.nick + ' (' + f.hp + 'hp)').join('   ·   '), rsoW/2, rsoH/2 - 6, rsoW - 80, 16);
  const bw = 230, bh = 50, bx = rsoW/2 - bw/2, by = rsoH/2 + 36;
  ctx.fillStyle = '#53fc18'; rsoRoundRect(ctx, bx, by, bw, bh, 10); ctx.fill();
  ctx.fillStyle = '#000'; ctx.font = 'bold 19px Inter, sans-serif';
...
function rsoUpdate(dt) {
  ...
  for (const t of rsoTracers) t.life -= dt;
  rsoTracers = rsoTracers.filter(t => t.life > 0);
  for (const fl of rsoFloats) { fl.life -= dt; fl.y -= dt*0.03; }
  rsoFloats = rsoFloats.filter(fl => fl.life > 0);

  const now = performance.now();
  for (const f of alive) {
    let enemy = null, nd = Infinity;
    for (const o of alive) { if (o===f) continue; const d = Math.hypot(o.x-f.x,o.y-f.y); if (d<nd){nd=d;enemy=o;} }
    f.target = enemy; if (!enemy) continue;
    const hasLOS = !rsoLineBlocked(f.x, f.y, enemy.x, enemy.y);
    const wantAng = Math.atan2(enemy.y-f.y, enemy.x-f.x);
    ...
    if (now > f.decisionAt) {
      ...
      else if (f.mode === 'peek') { f.anchorCover = rsoNearCover(f); f.moveTarget = f.anchorCover ? rsoPeek(f.anchorCover, enemy) : rsoStrafe(f, enemy); }
    ...
    if (!flashed && hasLOS && !f.reloading && nd < RAK.range && aimErr < 0.25) {
      ...
      if (f.burstLeft > 0 && f.shotTimer <= 0 && f.ammo > 0) {
        ...
        f.spread = Math.min(RAK.maxSpread, f.spread + RAK.spreadPerShot);
        if (f.burstLeft <= 0) f.burstCooldown = 250 + royFloat()*500;
      }
    } else f.spread = Math.max(RAK.baseSpread, f.spread - RAK.spreadRecover);
    if (f.burstCooldown > 0) f.burstCooldown -= dt;

    if (f.moveTarget) {
      const mdx = f.moveTarget.x-f.x, mdy = f.moveTarget.y-f.y, md = Math.hypot(mdx,mdy)||1;
...
function rsoFlank(f, e) { const a = Math.atan2(e.y-f.y, e.x-f.x) + (f.strafeDir>0?0.9:-0.9), d = 90+royFloat()*80; return { x: e.x-Math.cos(a)*d, y: e.y-Math.sin(a)*d }; }
function rsoNearCover(f) { let best=null,bd=Infinity; for (const c of rsoCovers) { const d=Math.hypot(c.x-f.x,c.y-f.y); if (d<bd&&d<200){bd=d;best=c;} } return best; }
function rsoHideSpot(f, e) { const c = rsoNearCover(f); if (!c) return null; const a = Math.atan2(c.y-e.y,c.x-e.x), cr = (c.r||Math.max(c.w,c.h)/2)+f.radius+5, hx = c.x+Math.cos(a)*cr, hy = c.y+Math.sin(a)*cr; return rsoCoverAt(hx,hy,f.radius-2)?null:{x:hx,y:hy}; }
function rsoPeek(cv, e) { const a = Math.atan2(cv.y-e.y,cv.x-e.x), cr = (cv.r||Math.max(cv.w,cv.h)/2)+22, side = (royFloat()<0.5?1:-1)*0.7; return { x: cv.x+Math.cos(a+side)*cr, y: cv.y+Math.sin(a+side)*cr }; }
...
function rsoEnd(winner) {
  ...
  setTimeout(() => {
    ...
    if (winner) { const wp = royPlayers[winner.nick]; if (wp) { wp.alive = true; wp.dying = false; wp.removed = false; royRender(); royDeclareWinner(wp); } }
    else { royPhase = 'finished'; royStatus('Ничья!'); royRender(); }
  }, 3500);
}
